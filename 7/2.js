...
while (true) {
  currentProgram.weightWithChildrens = 0;
  if (currentProgram.childrens) {
    currentProgram.weightWithChildrens += currentProgram.weight;
    //calc
    let calcComplete = true;
    for (child of currentProgram.childrens) {
      let childObject = programs.filter(p => p.name === child)[0];
      if (childObject.weightWithChildrens) {
        currentProgram.weightWithChildrens += programs.filter(
          p => p.name === child
        )[0].weightWithChildrens;
      } else {
        currentProgram = programs.filter(p => p.name === child)[0];
        calcComplete = false;
        break;
      }
    }
    if (calcComplete) {
      let weightRes = isChildrensWeightsEven(currentProgram);
      if (weightRes.res) {
        currentProgram = findParent(currentProgram);
      } else {
        console.log(weightRes.weights);
        for (child of currentProgram.childrens) {
          let childObject = programs.filter(p => p.name === child)[0];
...
function calcProgramWeightWithChildrens(program) {
  let weight = 0;
  weight += program.weight;
  if (program.childrens) {
    for (child of program.childrens) {
      weight += programs.filter(p => p.name === child)[0].weight;
    }
  }
  return weight;
}

function isChildrensWeightsEven(program) {
  if (program.childrens) {
    let weights = program.childrens.map(
      c => programs.filter(p => p.name === c)[0].weightWithChildrens
    );

    return {
      res: !!weights.reduce(function(a, b) {
        return a === b ? a : NaN;
      }),
      weights
    };
  }
  return { res: false, weights: null };
}

function findParent(program) {
  return programs.filter(
    p => p.childrens && p.childrens.includes(program.name)
  )[0];
}
