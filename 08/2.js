...
let instructions = input.split('\n').map(e => {
  const s = e.split(' ');
  const register = s[0];
  const operation = { type: s[1], value: +s[2] };
  const condition = { left: s[4], operator: s[5], right: +s[6] };
  return {
    register,
    operation,
    condition,
...
let maxRegister = { name: null, value: 0 };

for (let instruction of instructions) {
  const register = findRegister(instruction.register);
  if (
    checkCondition(
      findRegister(instruction.condition.left).value,
      instruction.condition.operator,
      instruction.condition.right,
    )
  ) {
    if (instruction.operation.type === 'inc') {
      register.value += instruction.operation.value;
    } else if (instruction.operation.type === 'dec') {
      register.value -= instruction.operation.value;
    }

    if (register.value > maxRegister.value) {
      maxRegister = Object.assign({}, register);
    }
  }
}

console.log('registers ', registers);
console.log('max registers', findMaxRegister(registers));
console.log('max register ever', maxRegister);

function findRegister(name) {
  return registers.filter(r => r.name === name)[0];
}

function checkCondition(left, operation, right) {
  switch (operation) {
    case '>=':
      return left >= right;
    case '<=':
...
function findMaxRegister(registers) {
  let maxValue = Math.max.apply(null, registers.map(r => r.value));
  return registers.filter(r => r.value === maxValue);
}
