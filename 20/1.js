const input = `p=<-317,1413,1507>, v=<19,-102,-108>, a=<1,-3,-3>
...
while (iters < 1e4) {
  let minDistanceToZero = Number.MAX_SAFE_INTEGER;
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    if (distanceToZero(p.p) < minDistanceToZero) {
      minDistanceToZero = distanceToZero(p.p);
      closestParticle = i;
    }
  }
  closestCount[closestParticle]
    ? closestCount[closestParticle]++
    : (closestCount[closestParticle] = 1);
  iters++;
}

console.log(closestCount);

function distanceToZero(p) {
